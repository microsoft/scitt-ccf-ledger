...
# Make the installed image reproducible: installing packages leaves state that
# differs between two builds of the same sources - databases carrying insertion
# order and wall-clock timestamps, caches keyed on when they were written, and
# trust bundles concatenated in readdir order - none of it package content, but
# all of it changing the layer digest.
#
# Run this once after the last tdnf command; it rewrites that state
# deterministically rather than deleting it, so the image keeps a queryable
# package database and a working `tdnf history`, and leaves ownership,
# permissions and timestamps to canonicalize-rootfs.sh.
...
# SQLite's file-change counter and version-valid-for fields count write
# transactions rather than content and survive VACUUM, so two databases holding
# identical rows still differ here. Both are big-endian header fields, set once
...
# These bundles are generated by update-ca-trust at install time, which
# concatenates its sources in readdir order, so pinning the snapshot time fixes
# which certificates are installed but not the order they land in: the same
# three files enumerate as (distrusted, base, microsoft) on overlayfs and as
# (microsoft, base, distrusted) on ext4, reordering all 235 certificates.
...
# The RPM database is a SQLite file whose row order and primary keys follow the
# order packages were installed in, so renumber the packages by content and
# rewrite every index in a fixed order.
#
# `rpm --rebuilddb` is not a substitute: it keeps the existing ordering, fails
# outright here when replacing the old database, and would neither drop the
# scitt record nor reset the SQLite counters.
...
# tdnf stamps every transaction with the wall clock, but emptying the file
# leaves no schema either, so `tdnf history` then fails with "no such table"
# instead of reporting an empty history. `tdnf history init` rebuilds it from
# the installed packages and only writes when the schema is missing, so the
# truncation has to come first.

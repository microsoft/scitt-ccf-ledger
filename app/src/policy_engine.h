  ...
  namespace js
  {
      ...
      ccf::js::core::Context& ctx, const scitt::cose::ProtectedHeader& phdr)
    {
...

      {
        ...
        {
          JS_CHECK_OR_THROW(obj.set_int64("alg", phdr.alg.value()));
        }
        ...
        if (phdr.crit.has_value())
        {
          ...
          for (const auto& e : phdr.crit.value())
          {
            ...
            {
              JS_CHECK_OR_THROW(crit_array.set_at_index(
                i++,
          ...
          JS_CHECK_OR_THROW(obj.set("crit", std::move(crit_array)));
        }

        if (phdr.kid.has_value())
        {
          JS_CHECK_OR_THROW(obj.set("kid", ctx.new_string(phdr.kid.value())));
        }
        ...
        {
          JS_CHECK_OR_THROW(obj.set("feed", ctx.new_string(phdr.feed.value())));
        }

        if (phdr.iat.has_value())
        {
          JS_CHECK_OR_THROW(obj.set_int64("iat", phdr.iat.value()));
        }

        if (phdr.svn.has_value())
        {
          JS_CHECK_OR_THROW(obj.set_int64("svn", phdr.svn.value()));
        }
        ...
        {
          JS_CHECK_OR_THROW(cwt.set_int64("iat", phdr.cwt_claims.iat.value()));
        }
        if (phdr.cwt_claims.svn.has_value())
        {
          JS_CHECK_OR_THROW(cwt.set_int64("svn", phdr.cwt_claims.svn.value()));
        }
        ...
        if (phdr.tss_map.cose_key.has_value())
        {
          ...
          JS_CHECK_OR_THROW(tss_map.set("cose_key", std::move(cose_key_obj)));
        ...
        {
          JS_CHECK_OR_THROW(tss_map.set_int64("ver", phdr.tss_map.ver.value()));
        }
        JS_CHECK_OR_THROW(obj.set("attestedsvc", std::move(tss_map)));
      }
      ...
      const std::optional<verifier::VerifiedSevSnpAttestationDetails>& details)
    {
      ...
      if (details.has_value())
      {
        ...
        JS_CHECK_OR_THROW(obj.set("host_data", ctx.new_string(host_data_str)));
        if (details->get_uvm_endorsements().has_value())
        {
          ...
          JS_CHECK_OR_THROW(obj.set("uvm_endorsements", std::move(uvm_obj)));
        }
        ...
        {
          JS_CHECK_OR_THROW(reported_tcb.set_uint32("snp", tcb.snp.value()));
        }
        if (tcb.tee.has_value())
        {
          JS_CHECK_OR_THROW(reported_tcb.set_uint32("tee", tcb.tee.value()));
        }
        ...
        {
          JS_CHECK_OR_THROW(reported_tcb.set_uint32("fmc", tcb.fmc.value()));
        }
        if (tcb.hexstring.has_value())
        {
          JS_CHECK_OR_THROW(reported_tcb.set(
            "hexstring", ctx.new_string(tcb.hexstring.value())));
        }
        JS_CHECK_OR_THROW(obj.set("reported_tcb", std::move(reported_tcb)));
        JS_CHECK_OR_THROW(obj.set(
          "product_name",
          ctx.new_string(

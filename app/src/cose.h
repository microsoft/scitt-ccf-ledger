...
namespace scitt::cose
{
  ...
  std::optional<std::variant<int64_t, std::vector<uint8_t>>> crv_n_k_pub() const
  {
    return crv_n_k_pub_;
  }
...
static ProtectedHeader decode_protected_header(QCBORDecodeContext& ctx)
{
  ...
  cose_key_items[TSS_COSE_KEY_CRV_N_K_PUB_INDEX].uLabelType = QCBOR_TYPE_INT64;
  ...
  {
    throw COSEDecodeError("Content-type must be of type text string or int64");
  }
  ...
  if (header_items[TSS_INDEX].uDataType != QCBOR_TYPE_NONE)
  {
    ...
    if (tss_items[TSS_COSE_KEY_INDEX].uDataType != QCBOR_TYPE_NONE)
    {
      ...
      if (cose_key_items[TSS_COSE_KEY_KTY_INDEX].uDataType != QCBOR_TYPE_NONE)
      {
        ...
        if (cose_key_items[TSS_COSE_KEY_X_E_INDEX].uDataType != QCBOR_TYPE_NONE)
        {
          parsed.tss_map.cose_key->set_x_e(
            cbor::as_vector(cose_key_items[TSS_COSE_KEY_X_E_INDEX].val.string));
        }
